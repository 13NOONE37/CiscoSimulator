...
export default function SystemTime({ t, config }) {
   const [currentTimeSource, setcurrentTimeSource] = useState(
      config.timeSource
   );
   const [firstServer, setfirstServer] = useState(config.firstNTP);
   const [secoundServer, setsecoundServer] = useState(config.secoundNTP);
   const [updateRate, setupdateRate] = useState(config.updateRate);
   const [currentTime, setcurrentTime] = useState(config.currentTime);
   const [currentTimeZone, setcurrentTimeZone] = useState(config.timeZone);

   const [tempDate, settempDate] = useState(0);
   const [tempTime, settempTime] = useState(0);

   const handleSetLocalTime = () => setcurrentTime(new Date());

   const handleSubmit = (e) => {
      e.preventDefault();
      if (tempDate != 0 && tempTime != 0) {
         setcurrentTime(new Date(`${tempDate} ${tempTime}`));
         config.currentTime = new Date(`${tempDate} ${tempTime}`);
      } else {
         config.currentTime = currentTime;
      }
      config.timeSource = currentTimeSource;
      config.firstNTP = firstServer;
      config.secoundNTP = secoundServer;
      config.updateRate = updateRate;
      config.timeZone = currentTimeZone;
   };
      ...
      <article>
         <div className='tplinkBoxBase1'>
            ...
            <div className='timeTableInfo'>
               <span>
                  Current System Date: {new Date(currentTime).toDateString()}
               </span>
               <span>Current Time Source: {currentTimeSource}</span>
            </div>
            <div className='InfoTableTitle'>{t('Info_SystemTimeConfig')}</div>
            <div className='systemTimeBox'>
               <span>
                  <input
                     type='radio'
                     name='timeSource'
                     value='Manual'
                     defaultChecked={currentTimeSource == 'Manual'}
                     onChange={(e) =>
                        handleGlobalChange(e, setcurrentTimeSource)
                     }
                  />
                  Manual
                  <span className='timeBoxInputs'>
                     <div>
                        Date:
                        <input
                           type='date'
                     ...
                     <div>
                        ...
                        <input
                           ...
                           step={1}
                           disabled={currentTimeSource != 'Manual'}
                           onChange={(e) => handleGlobalChange(e, settempTime)}
                        />
                     </div>
                  </span>
               </span>
               <span>
                  <input
                     ...
                     name='timeSource'
                     value='NTP'
                     defaultChecked={currentTimeSource == 'NTP'}
                     onChange={(e) =>
                        handleGlobalChange(e, setcurrentTimeSource)
                     }
                  />
                  ...
                  <form onSubmit={handleSubmit} className='tplinkFormBase1'>
                     <span>
                        ...
                        <select
                           ...
                           onChange={(e) =>
                              handleGlobalChange(e, setcurrentTimeZone)
                           }
                        >
                     ...
                     <span>
                        ...
                        <ReactInputMask
                           ...
                           value={firstServer}
                           onChange={(e) =>
                              handleGlobalChange(e, setfirstServer)
                           }
                        ></ReactInputMask>
                        <input
                           type='submit'
                     ...
                     <span>
                        ...
                        <ReactInputMask
                           ...
                           value={secoundServer}
                           onChange={(e) =>
                              handleGlobalChange(e, setsecoundServer)
                           }
                        ></ReactInputMask>
                        <input
                           type='submit'
                     ...
                     <span>
                        ...
                        <input
                           ...
                           value={updateRate}
                           onChange={(e) =>
                              handleGlobalChange(e, setupdateRate)
                           }
                           disabled={currentTimeSource != 'NTP'}
                        />
                        <input
               ...
               <span>
                  <input
                     ...
                     name='timeSource'
                     value='Local'
                     defaultChecked={currentTimeSource == 'Local'}
                     onChange={(e) => {
                        handleGlobalChange(e, setcurrentTimeSource);
                        handleSetLocalTime();
                     }}
